...
bool noiseBlankerAllowed(DemodMode mode) noexcept
{
    return mode == DemodMode::Usb || mode == DemodMode::Lsb
        || mode == DemodMode::DigU || mode == DemodMode::DigL
        || mode == DemodMode::Dsb
        || mode == DemodMode::Fm || mode == DemodMode::Nfm;
}
...
bool ChannelSettings::operator==(const ChannelSettings &o) const noexcept
{
    return offsetHz == o.offsetHz && mode == o.mode && filterLowHz == o.filterLowHz
        ...
        && noiseBlankerEnabled == o.noiseBlankerEnabled
        && noiseBlankerThresholdDb == o.noiseBlankerThresholdDb
...
bool ChannelProcessor::configureForMode()
{
    ...
    if (!m_ctcss.configure(m_channelRate, m_settings.ctcssToneHz))
        return false;
    if (!m_noiseBlanker.configure(m_channelRate))
        return false;
    m_noiseBlanker.setThresholdDb(m_settings.noiseBlankerThresholdDb);
...
void ChannelProcessor::applySettings(const ChannelSettings &settings)
{
    ...
    const bool noiseBlankerChanged = settings.noiseBlankerEnabled != m_settings.noiseBlankerEnabled
        || settings.noiseBlankerThresholdDb != m_settings.noiseBlankerThresholdDb;
    ...
    if (noiseBlankerChanged) {
        m_noiseBlanker.setThresholdDb(m_settings.noiseBlankerThresholdDb);
        m_noiseBlanker.reset();
    }
                                              ...
                                              float *stereoOut) noexcept
{
    ...
    while (offset < n) {
        ...
        if (m_settings.noiseBlankerEnabled && noiseBlankerAllowed(m_settings.mode))
            m_noiseBlanker.process(m_filtered.data(), decimated);
