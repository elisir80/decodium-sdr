...
- Il DSP ora aspetta il primo canale prima di svuotare una nuova sorgente e
  la sessione ne conferma la creazione nel thread DSP; inoltre accorpa le
  notifiche IQ/audio prima della propria coda e suddivide una raffica in
  brevi turni dell'event loop. La creazione di un canale, i cambi filtro e i
  moduli IQ non possono più perdere i primi frame o restare bloccati dietro
...
- The DSP now waits for the first channel before draining a newly attached
  source, and the session confirms its creation in the DSP thread. It also
  coalesces IQ/audio notifications before its event queue and splits a burst
  into short event-loop turns. Channel creation, filter changes and IQ
  modules can no longer lose their first frames or be starved behind a
